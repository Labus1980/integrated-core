...
*/
  ...
  function Plugin(element, options) {
    ...
    var scriptElement = document.getElementById('zammad_form_script')
    this._src = scriptElement ? scriptElement.src : ''

    var baseUrlOverride = ''
    if (scriptElement && scriptElement.getAttribute) {
      baseUrlOverride = scriptElement.getAttribute('data-zammad-base-url') || ''
    }

    if (baseUrlOverride) {
      var normalizedBaseUrl = baseUrlOverride.replace(/\/$/, '')
      this.css_location = normalizedBaseUrl + this._css_location
      this.endpoint_config = normalizedBaseUrl + this._endpoint_config
      this.endpoint_submit = normalizedBaseUrl + this._endpoint_submit
    }
    else if (this._src) {
      var baseUrl = this._src.replace(this._script_location, '')
      this.css_location = baseUrl + this._css_location
      this.endpoint_config = baseUrl + this._endpoint_config
      this.endpoint_submit = baseUrl + this._endpoint_submit
    }
    else {
      this.css_location = this._css_location
      this.endpoint_config = this._endpoint_config
      this.endpoint_submit = this._endpoint_submit
    }
